...
//  NoticeApp
//
//  Created by TJOEUN-304 on 2020/03/13.
...
class ListController: UITableViewController {
    ...
    //웹서버에서 데이터 가져오기
    //비동기로 가져와야 한다.
    //메인 실행부는 UI처리와 이벤트를 담당하기 떄문에,
    //절대로 푸르나 스트림으로 인한 대기상태에 빠지게 해서는 안된다.
    //앱이 멈춘다.
    //비동기 객체 URLSession
    func loadData(){
        ...
        //{(파라미터) -> 반환형 in 로직} =>자바에서는 람다, iOS에서는 클로저
        let task = urlSession.dataTask(with: url, completionHandler: {
            (data, response, error) in
            //여기에 로직
            //테이블뷰 UI를 다시 새로고침
            //UI를 갱신하는 권한은 메인 실행부에만 있으므로,
            //이 영역에서 화면 갱신이 불가능하다.
            //해결책은
            //메인 실행부에게 갱신을 부탁
            ...
            //테이블에서 접근하기 좋게 하려고
            let parser = JSONDecoder() //json 해석자
            do{
               //파싱이 완료된 시점에서 반환되는 것은 NoticeList 클래스의 인스턴스이다.
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Configure the cell...
        let notice = noticeArray[indexPath.row] //배열에 요소 꺼내기
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            //모델을 삭제(데이터제거)
            var notice = noticeArray[indexPath.row] //곧 지워질 데이터!
